...
def teacher_dashboard():
    ...
    with c2:
        ...
        if st.button("Log Out",type='secondary',key='loginbackbtn',shortcut='control+backspace',width='content'):
...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning("You haven't created any subjects yet.Please create a subject to begin !")
    ...
    with col1:
        seleted_subject_label=st.selectbox('Select Subject',options=list(subject_options.keys()))
    with col2:
        if st.button("Add Photos",type='primary',width='stretch',icon=":material/photo_library:"):
            add_photos_dialog()

    seleted_subject_id=subject_options[seleted_subject_label]
    ...
    with c2:
        ...
        if st.button("Run analysis",width='stretch',type='secondary',icon=":material/analytics:",disabled=not has_photos):
            with st.spinner("Deep scanning classroom Photos..."):
                ...
                enrolled_res=supabase.table('subject_students').select("*,students(*)").eq('subject_id',seleted_subject_id).execute()
    ...
    with c3:
        if st.button("Use Voice Attendance",width='stretch',type='primary',icon=":material/mic:"):
            Voice_attendance_dialog(seleted_subject_id)
